<x-app-layout>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div>
                ...
                @endif
                <form action="{{ route('dashboard.my-address.update', $address->id) }}" method="post" class="w-full"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Full
                                name</label>
                            <input type="text" value="{{ old('full_name') ?? $address->full_name }}" name="full_name"
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            <label class="block uppercase tracking-wide text-gray-700 text-xs font-bold mb-2">Phone
                                number</label>
                            <input type="number" value="{{ old('telphone_number') ?? $address->telphone_number }}"
                                name="telphone_number" placeholder="Phone Number"
                                class="block w-full bg-gray-200 text-gray-700 border-gray-200 rounded py-3 px-4 leading-text focus:ooutline-none focus:bg-white focus:border-gray-500">
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <select name="type_of_place"
                                class="block w-full bg-gray-200 text-gray-700 border-gray-200 rounded py-3 px-4 leading-text focus:ooutline-none focus:bg-white focus:border-gray-500">
                                <option value="{{ $address->type_of_place }}">
                                    {{ $address->type_of_place == 0 ? 'Home' : 'Office' }} </option>
                    ...
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <textarea name="detail_address"
                                class="block w-full bg-gray-200 text-gray-700 border-gray-200 rounded py-3 px-4 leading-text focus:ooutline-none focus:bg-white focus:border-gray-500"> {!! old('detail_address') ?? $address->detail_address !!} </textarea>
                        </div>
                    </div>
                    <div class="flex flex-wrap -mx-3 mb-6">
                        <div class="w-full px-3">
                            ...
                            <input type="text" id="searchLocation" data-initial-value="{{ $city }}"
                            ...
                            <input type="hidden" id="city_id" name="city_id"
                                data-initial-value-city-id="{{ $address->city }}">
                            <input type="hidden" id="province_id" name="province_id"
                                data-initial-value-province-id="{{ $address->provinces }}">
                        </div>
                    </div>
                    <div class="flex
                                flex-wrap -mx-3 mb-6">
    ...
    <script>
        $(document).ready(function() {
            const initialSearchLocation = $('#searchLocation').data('initial-value');
            if (initialSearchLocation !== '') {
                $('#searchLocation').val(initialSearchLocation);
            }
        });
        $(document).ready(function() {
            const initialIdCity = $('#city_id').data('initial-value-city-id');
            if (initialIdCity !== '') {
                $('#city_id').val(initialIdCity);
            }
        });
        $(document).ready(function() {
            const initialIdProvince = $('#province_id').data('initial-value-province-id');
            if (initialIdProvince !== '') {
                $('#province_id').val(initialIdProvince);
            }
        });
        $(document).ready(function() {
            const initialPlace = $('#province_id').data('initial-value-province-id');
            if (initialIdProvince !== '') {
                $('#province_id').val(initialIdProvince);
            }
        });
