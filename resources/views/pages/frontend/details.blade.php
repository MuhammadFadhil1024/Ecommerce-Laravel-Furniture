...
@section('content')
        ...
        <div class="flex flex-wrap my-4 md:my-12">
            ...
            <div class="flex-1 px-4 md:p-6">
                ...
                <div class="flex justify-start mb-2">
                    <div class="text-xl">IDR</div>
                    <div class="text-xl px-6">{{ number_format($product->price) }}</div>
                </div>
                <div class="flex flex-wrap items-center mb-2">
                    <p class="text-xl">Quantity</p>
                    <div class="flex px-6">
                        <button class="font-semibold text-lg px-4 border border-gray-600" id="decraseQuantity">-</button>
                        <h6 class="font-semibold text-lg px-4 border border-gray-600" id="quantity" name="quantitiy">1</h6>
                        <button class="font-semibold text-lg px-4 border border-gray-600" id="incraseQuantity">+</button>
                    </div>
                </div>

                @if ($product->stock == 0)
                    <p class="text-xl text-red-600">Sold out</p>
                @else
                    <p class="text-xl text-red-600">Current stock <span>{{ $product->stock }}</span> pcs</p>        
                @endif
                        ...
                        <div class="px-3 flex-none" style="width: 320px">
                            <div class="rounded-xl p-4 pb-8 relative bg-white">
                                <div class="rounded-xl overflow-hidden card-shadow w-full h-36">
                                    <img src="{{ $recomendation->galleries()->exists() ? Storage::url($recomendation->galleries->first()->url) : 'data:image/gif;base64,R0lGODlhAQABAIAAAMLCwgAAACH5BAAAAAAALAAAAAABAAEAAAICRAEAOw==' }}"
    ...
    <script>
        let decreaseButtonDisabled = false;

        document.getElementById('incraseQuantity').addEventListener('click', function() {
            ...
            document.getElementById('quantityValue').value = newQuantity;

            if (decreaseButtonDisabled) {
                $('#decraseQuantity').prop('disabled', false);
                decreaseButtonDisabled = false;
            }
        });

        $(document).ready(function() {
            $('#decraseQuantity').click(function() {
                let currentQuantity = parseInt($('#quantity').text());

                if (currentQuantity > 1) {
                    let newQuantityDecrease = currentQuantity - 1;
                    $('#quantity').text(newQuantityDecrease);
                    $('#quantityValue').val(newQuantityDecrease);
                } else {
                    $('#decraseQuantity').prop('disabled', true);
